...
cv::Mat maskObjects(
	...
	for (auto object : objects) {
		...
		cv::Mat objMask, obj;
		cv::Mat diff(
			image.rows - object.rows + 1,
			image.cols - object.cols + 1,
			CV_32FC1
		);
		...
		for (int i = 0; i < diff.rows; i++) {
			for (int j = 0; j < diff.cols; j++) {
				...
				cv::MatConstIterator_<cv::Vec3b> itWin = window.begin<cv::Vec3b>();
				cv::MatConstIterator_<cv::Vec3b> itObj = obj.begin<cv::Vec3b>();
				cv::MatConstIterator_<uint8_t> itMask = objMask.begin<uint8_t>();
...
int main() {
	...
	cv::Mat mask = cv::imread("../images/map/stitching/map-mask.png", cv::IMREAD_GRAYSCALE);
