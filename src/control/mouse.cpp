...
Mouse::Mouse() : display(XOpenDisplay(NULL))
{
	if (display == NULL)
...
void Mouse::leftClick(const cv::Point& position) {
	click(position, Button1);
}

void Mouse::rightClick(const cv::Point& position) {
	click(position, Button3);
}
